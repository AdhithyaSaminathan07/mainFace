...
import type * as faceapiType from 'face-api.js'; // Type-only import
import { useFaceApi } from '@/context/FaceApiContext';

interface FaceCameraProps {
    onFaceDetected?: (descriptor: Float32Array, image: string) => void;
    onFaceMatch?: (bestMatch: faceapiType.FaceMatch) => void;
    labeledDescriptors?: faceapiType.LabeledFaceDescriptors[];
    mode: 'enroll' | 'scan';
...
export default function FaceCamera({ onFaceDetected, onFaceMatch, labeledDescriptors, mode, showAdvancedVisuals, onQualityChange }: FaceCameraProps) {
    ...
    const { isModelsLoaded, error, faceApi } = useFaceApi();
    const [message, setMessage] = useState('Initializing camera...');
    const [faceMatcher, setFaceMatcher] = useState<faceapiType.FaceMatcher | null>(null);
    ...
    }, [isModelsLoaded, error, faceApi]);

    useEffect(() => {
        if (labeledDescriptors && labeledDescriptors.length > 0 && faceApi) {
            setFaceMatcher(new faceApi.FaceMatcher(labeledDescriptors, 0.6));
        } else {
            setFaceMatcher(null);
        }
    }, [labeledDescriptors, faceApi]);
